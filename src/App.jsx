...
import { Activity, Copy, Discord, Gamepad2, Image as ImageIcon, Server, Shield, Check } from "lucide-react";
...
function useDayZStatus() {
  const [data] = useState({ online: true, players: 24, max: 60 });
    ...
    <header className="relative z-20">
      <Container className="flex items-center justify-between py-4">
        <a href="#" className="group inline-flex items-center gap-3">
          <img src={SITE.logoUrl} alt="Server logo" className="h-10 w-10 rounded-lg object-cover ring-1 ring-white/10" />
          <span className="text-lg font-semibold text-amber-300 group-hover:text-amber-200">{SITE.name}</span>
        </a>
        <nav className="hidden items-center gap-6 text-sm text-zinc-200/90 md:flex">
          ...
          <a className="inline-flex items-center gap-2 rounded-full border border-white/10 px-3 py-1.5 hover:border-amber-400/60"
             href={SITE.discordInvite} target="_blank" rel="noreferrer">
...
function Hero() {
  ...
  return (
    <div className="relative isolate">
      ...
      <div aria-hidden className="absolute inset-0 bg-gradient-to-b from-black/70 via-black/60 to-black" />
      <Nav />
        ...
        <div className="max-w-3xl">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <a href={SITE.discordInvite} target="_blank" rel="noreferrer"
               className="inline-flex items-center gap-2 rounded-xl border border-white/20 px-5 py-3 text-zinc-100 hover:bg-white/10">
              <Discord className="h-4 w-4" /> Join Discord
            </a>
          </div>
        </div>
      </Container>
...
function StatusBar() {
  ...
  return (
    <div className="mt-6 inline-flex items-center gap-3 rounded-full border border-white/10 bg-black/30 px-3 py-2 text-sm text-zinc-200/90 backdrop-blur">
      ...
      {error ? <span>Status unavailable</span> : online ? <span>Online • {players}/{max} players</span> : <span>Server offline</span>}
    </div>
  );
}
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
          ...
          <div className="flex items-center gap-2 text-sm">
            <input readOnly value={`${SITE.ip}:${SITE.gamePort}`} className="w-full rounded-lg bg-black/30 px-3 py-2" />
            <button onClick={copy} className="rounded-lg border border-white/20 px-3 py-2 text-sm hover:bg-white/10">
              Copy
            </button>
          </div>
          <a href={steamConnect} className="mt-3 block rounded-xl bg-amber-600 px-4 py-2 text-center text-white hover:bg-amber-700">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
          ...
          <a href={SITE.discordInvite} target="_blank" rel="noreferrer"
            ...
            <Discord className="inline h-4 w-4 -mt-1 mr-2" /> Join Discord
          </a>
        </div>
    ...
    <Section id="rules" eyebrow="Rules" title="Play fair. Survive smarter." icon={Shield}>
      <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
        <ul className="list-inside list-disc space-y-2 text-zinc-200/90">
          {SITE.rules.map((r, i) => <li key={i}>{r}</li>)}
        </ul>
      </div>
    </Section>
  );
...
function Gallery() {
  return (
    <Section id="gallery" eyebrow="Gallery" title="From the field" icon={ImageIcon}>
